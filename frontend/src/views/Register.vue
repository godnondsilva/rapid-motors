                    ...
                    <form @submit.prevent="handleSubmit(register)">
                        ...
                        <div class="form-group">
                            ...
                            <validation-provider rules="required|min:6|max:30" v-slot="{ errors }" vid="confirm_password">
                                <input name="password" type="password" id="password" v-model="password" placeholder="Enter your password">
                                <div class="text-danger push-right">{{ errors[0] }}</div>
                            </validation-provider>
                        </div>
                        <div class="form-group">
                            <label for="confirm_password">Confirm Password</label>
                            <validation-provider rules="required|confirmed:confirm_password" v-slot="{ errors }" >
                                <input type="password" id="confirm_password" v-model="confirm_password" placeholder="Re-enter your password">
                                <div class="text-danger push-right">{{ errors[0] }}</div>
                            </validation-provider>
                        </div>
                        <div class="form-group">
                            ...
                            <validation-provider rules="required|min:3|max:255" v-slot="{ errors }">
                                <textarea type="text" id="address" v-model="address" placeholder="Enter your address"></textarea>
                                <div class="text-danger push-right">{{ errors[0] }}</div>
                            </validation-provider>
                        </div>
                        <b-button @click="show=true" :disabled="invalid" class="btn btn-yellow push-right">Register</b-button>
                        
                        <b-modal v-model="show" id="register" title="Confirm Account Creation">
...
export default {
    ...
    methods: {
        ...
        register() {
            ...
            .then(() => {
                alert('Your account has been sucessfully created! Please login to continue');
