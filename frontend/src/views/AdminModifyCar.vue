                    ...
                    <form @submit.prevent="handleSubmit(saveCar)">
                        <div class="form-group">
                            ...
                            <validation-provider rules="required|min:3|max:30" v-slot="{ errors }">
                                <input type="text" id="name" v-model="localCar.name" :disabled="isDisabled" placeholder="Enter the name of the car">
                        ...
                        <div class="form-group">
                            ...
                            <validation-provider rules="required|year_limit:4" v-slot="{ errors }">
                                <input type="text" id="year" v-model="localCar.year" :disabled="isDisabled" placeholder="Enter the model year of the car">
                                <div class="text-danger push-right">{{ errors[0] }}</div>
                            </validation-provider>
                        </div>
                        <div class="form-group">
                            ...
                            <validation-provider v-slot="{ errors }">
                                <input type="file" id="image" name="image" :disabled="isDisabled" @change="updateImage" accept="image/png, image/jpeg, image/jpg">
                                <div class="text-danger push-right">{{ errors[0] }}</div>
                            </validation-provider>
                        </div>
                        <div class="form-group">
                            ...
                            <validation-provider rules="required|min:3|max:20" v-slot="{ errors }" >
                                <input type="text" id="color" v-model="localCar.color" :disabled="isDisabled" placeholder="Enter the color of the car">
                                <div class="text-danger push-right">{{ errors[0] }}</div>
                            </validation-provider>
                        </div>
                        <div class="form-group">
                            ...
                            <validation-provider rules="required|min_value:1000|max_value:99999999.99" v-slot="{ errors }">
                                <input type="text" id="price" v-model="localCar.price" :disabled="isDisabled" placeholder="Enter the price of the car in numbers">
                                <div class="text-danger push-right">{{ errors[0] }}</div>
                            </validation-provider>
                        </div>
                        <div class="form-group">
                        ...
                        <div>
                            <button v-if="isDisabled === true" class="btn btn-yellow push-right" @click.prevent="isDisabled=false">Update Car</button>
                            <b-button v-else @click="show=true" class="btn-bg btn-yellow push-right" :disabled="invalid">Save</b-button>
                        </div>

                        <b-modal v-model="show" id="car-modify-confirm" title="Confirm Car Modification">
                            <p class="my-4">Are your sure you want to save this car?</p>
...
export default {
    ...
    methods: {
        ...
        updateImage(e) {
            ...
            if(this.localCar.img_name === undefined || this.localCar.img_name === '') {
        ...
        checkUpdate() {
            if (this.localCar.model_id !== this.dummyCar.model_id ||
